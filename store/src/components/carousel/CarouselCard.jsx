    ...
    >
      {sliderData?.map((item, i) => (
        <SwiperSlide
          ...
          key={i + 1}
        >
          <div className="text-sm text-gray-600 hover:text-purple-dark dark:bg-zinc-900 relative">
            <Image
              width={2000}
              height={400}
              src={item.image}
          ...
          {(item.title || item.info || item.buttonName) && (
            <div className="absolute top-0 left-0 z-10 p-r-16 flex-col flex w-full h-full place-items-start justify-center">
              <div className="pl-4 pr-12 sm:pl-10 sm:pr-16 w-10/12 lg:w-8/12 xl:w-7/12">
                ...
                <Link
                  href={item.url}
                  className="hidden sm:inline-block lg:inline-block text-sm leading-6  font-medium mt-6 px-6 py-2 bg-purple-500 text-center rounded-md text-white hover:bg-purple-600"
                >
                  {item.buttonName}
                </Link>
              </div>
